...
int main(int argc, char *argv[])
{
  ...
  while (std::getline(file, line))
  {
    ...
    Formula *formula = proc_line(line);
    ret_form_vec->push_back(formula);
  }

  std::cout << "Welcome to Converter" << std::endl;
  Formula *formula = new_FConj(ret_form_vec);
  // print_formula(formula, true);
  to_dnf(&formula);
  std::cout << "Checking SAT" << std::endl;
  bool sat_result = sat(formula);
  std::cout << "SAT: " << (sat_result ? "true" : "false") << std::endl;
